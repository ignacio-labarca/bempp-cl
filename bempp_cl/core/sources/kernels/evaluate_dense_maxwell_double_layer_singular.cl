...
__kernel void kernel_function(
  ...
  for (uint quadIndex = localQuadPointsPerItem * localId;
    ...
    /* Compute Laplace kernel value G(x,y) = 1/(4*pi*|x-y|) */
    KERNEL(novec)
    (testGlobalPoint, trialGlobalPoint, testNormal, trialNormal,
     kernel_parameters, &kernelValue);

    /* Compute gradient factor: -1/|x-y|^2 */
    REALTYPE3 diff = testGlobalPoint - trialGlobalPoint;
    REALTYPE dist = length(diff);
    REALTYPE gradFactor = -M_ONE / (dist * dist);

    /* Compute cross product and dot with diff:
       diff . (test x trial) * G / |x-y|^2
       This gives the curl term: <test, curl G x trial> */
    for (i = 0; i < 3; ++i)
      for (j = 0; j < 3; ++j) {
        REALTYPE3 crossProd = cross(testElementValue[i], trialElementValue[j]);
        shapeIntegral[i][j] +=
            kernelValue * gradFactor *
            dot(diff, crossProd) * weight;
      }
  }
