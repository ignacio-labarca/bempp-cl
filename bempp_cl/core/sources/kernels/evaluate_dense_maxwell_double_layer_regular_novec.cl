...
__kernel void kernel_function(
  ...
  for (testQuadIndex = 0; testQuadIndex < NUMBER_OF_QUAD_POINTS;
    ...
    for (trialQuadIndex = 0; trialQuadIndex < NUMBER_OF_QUAD_POINTS;
      ...
      /* Compute Laplace kernel G(x,y) = 1/(4*pi*|x-y|) */
      KERNEL(novec)
      ...
      /* gradFactor = G(x,y) * (-1) / |x-y|^2 */
      REALTYPE factor = kernelValue * (-M_ONE) / (dist * dist) *
                        quadWeights[trialQuadIndex];

      /* Accumulate: factor * diff . (test_i x trial_j) for each i,j */
      for (i = 0; i < 3; ++i)
        for (j = 0; j < 3; ++j) {
          REALTYPE3 crossProd = cross(testElementValue[i], trialElementValue[j]);
          tempResult[i][j] += factor * dot(diff, crossProd);
        }
    }
