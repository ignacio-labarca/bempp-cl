...
# pylint: disable=too-many-locals
def nc1_tangential_trace_inverse(fenics_space):
    """
    Return the inverse NC1 (twisted) tangential trace operator.

    This function returns a pair (space, inverse_trace_matrix),
    where space is a Bempp space object and inverse_trace_matrix is the
    corresponding matrix that maps the coefficients of a Bempp RWG function
    to FEniCS Nedelec coefficients (inverse of nc1_tangential_trace).
    ...
    # For the inverse operation, we transpose the matrix structure
    # and apply inverse scaling
    inverse_trace_matrix = trace_matrix.T.tocoo()
    ...
    for i, j in zip(non_z[0], non_z[1]):
        ...
        # Apply inverse scaling: bempp_values / cross instead of cross / bempp_values
        inverse_trace_matrix[j, i] = bempp_values[k, 0] / cross[k]

    # Convert back to csc for efficient operations
    inverse_trace_matrix = inverse_trace_matrix.tocsc()

    # Now return everything
    return space, inverse_trace_matrix
